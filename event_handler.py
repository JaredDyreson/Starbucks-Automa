...
class GoogleEventHandler():
  ...
  def add_events(self, event: time_struct):
    start, end, event_id, status = self.check_event_presence(event.begin.replace(hour=0, minute=0, second=0, microsecond=0), event.summary)
    if(start is None and end is None):
      ...
      if not(original_ == event):
        ...
        success_message_ = "[+] Sucessfully updated event to {} {}".format(' '.join(original_.google_date_added_string().split()[1:]), event.google_date_added_string())
        print(colored(success_message_, 'green', 'on_grey'))
      else:
...
s = datetime.today().replace(hour=9, minute=0, second=0)
e = datetime.today().replace(hour=19, minute=0, second=0)

most_recent = ts.time_struct(s, e, "Jared's Work")

event_handler.add_events(most_recent)
