...
class Shift():
    ...
    def components(self, time: str) -> tuple:
        ...
        time_split = re.compile("((?P<start>\d*\:?\d*)?(?P<sstate>(AM|PM)))\-((?P<end>\d*\:?\d*)?(?P<estate>(AM|PM)))").match(time_scheduled)
        return ((time_split.group("start"), time_split.group("sstate")), (time_split.group("end"), time_split.group("estate")))
