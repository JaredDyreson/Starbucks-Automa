...
class BuildConfig:
    ...
    def __init__(self, version, state_path):
        if not (
...
class State:
    ...
    def write(self):
        with open(self.build_config.state_path, "w") as f:
            document = {"version": str(self.version), "configured": self.is_configured}
...
def read_state(build_config):
    ...
    try:
        ...
        raise EnvironmentError(
            "state file not found, you must run $ starbucksautoma init"
        )
...
class initializer:
    ...
    def ensure_utils(self):
        ...
        for utility in utilities:
            try:
                if (
                    subprocess.run(
                        ["which", utility], stdout=subprocess.DEVNULL
                    ).returncode
                    != 0
                ):
                    raise EnvironmentError(
                        "command {} cannot be found, is this system running systemd?".format(
                            utility
                        )
                    )
    ...
    def currently_logged_in(self) -> list:
        return [
            user
            for user in subprocess.check_output(["users"], encoding="utf-8").split("\n")
            if user
        ]
    ...
    def make_user_config(self) -> None:
        ...
        tz = (
            os.popen("timedatectl status | awk '/Time zone/ {print $3}'").read().strip()
        )
        ...
        new_state = State(self.build_config, self.build_config.version, True)
        new_state.write()
